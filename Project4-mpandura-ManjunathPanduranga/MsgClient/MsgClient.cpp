...
HttpMessage MsgClient::makeMessage(size_t n, const std::string& body, const EndPoint& ep)
{
  ...
  case 1:
    ...
    msg.addAttribute(HttpMessage::parseAttribute("fromAddr:" + this->endPt));
...
bool MsgClient::sendFile( const std::string& filenamewithwholeMessage, Socket& socket)
{
  // assumes that socket is connected
	std::string filename = findMsgAttribute(filenamewithwholeMessage, "filePath");
  ...
  msg.addAttribute(HttpMessage::Attribute("catg",findMsgAttribute(filenamewithwholeMessage,"catg")));
...
void MsgClient::Point_end(EndPoint port)
{
	endPt = "localhost:" + port;
}

void MsgClient::Channel_text(std::string msgType, std::string bareMessage, Socket & socket)
{
	std::string toAddr = "localhost:" + findMsgAttribute(bareMessage, "toPort");
	Point_end(findMsgAttribute(bareMessage, "fromPort"));
	HttpMessage httpMesssage = makeMessage(1, bareMessage, toAddr);
	sendMessage(httpMesssage, socket);
}

//finds the value of attr in msg
std::string MsgClient::findMsgAttribute(std::string msgBody, std::string attribute)
{
...
void MsgClient::Message_client(std::string bareMessage, Socket & socket)
{
	std::string path = findMsgAttribute(bareMessage, "filePath");
	sendFile(bareMessage, socket); //1st parameter should contain the path also
}


void MsgClient::ID_msgClient(EndPoint id)
{
	IDclient = id;
}
